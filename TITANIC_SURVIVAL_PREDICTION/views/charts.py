...
with st.container(border=True):
    ...
    st.caption(":rainbow[First class passengers had a much higher survival rate than second or third class.]")
...
with st.container(border=True):
    ...
    st.caption(":rainbow[Women were more likely to survive the titanic sex and survival has the most positive correlation]")

with st.container(border=True):
    st.subheader('Survival rate by place of embarkment')
    raw_df = pd.read_csv(os.path.join(base_dir, '..', 'TITANIC.csv'))
    ...
    st.caption(":rainbow[The Passengers embarking from Southampton were generally wealthier which correlated with their survival]")
